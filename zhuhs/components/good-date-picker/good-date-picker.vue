					...
					<view class="date">
						<view
							...
							:class="{
								...
								choosed: !x.isSpace && (x.isChoosed || x.isRangeStart || x.isRangeEnd),
...
export default {
	...
	methods: {
		...
		oneChooseFirstInit() {
			//首次引用单选初始化
			this.clearOneChoose();
			console.log('this.chooseOneDate:',this.chooseOneDate)
		...
		totalDateInit() {
			...
			let _initEndDate = this.option.initEndDate ? this.option.initEndDate : this.getNextMonth(6);
			const _dateNum = this.getMonthBetween(_initStartDate, _initEndDate);
			console.log(_initStartDate)
			console.log(_initEndDate)
			console.log(_dateNum)
		...
		.list {
			> .date {
				...
				> view {
					...
					&.range_space {
						background-color: rgba(0, 96, 228, .3);
						> view {
							color: #333;
						}
					}
